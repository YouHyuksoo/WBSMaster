/**
 * @file scripts/import-as-is-hns.ts
 * @description
 * V_HNS 사업부 AS-IS 분석 데이터 가져오기 스크립트
...
async function main() {
  console.log("🚀 V_HNS AS-IS 데이터 가져오기 시작...\n");
  ...
  const sheet = workbook.Sheets["V_HNS"];
  if (!sheet) {
    console.error(`❌ V_HNS 시트를 찾을 수 없습니다.`);
    console.log(`📋 사용 가능한 시트: ${Object.keys(workbook.Sheets).join(", ")}`);
    throw new Error("V_HNS 시트를 찾을 수 없습니다.");
  }
  ...
  if (!overview) {
    console.log("📁 V_HNS Overview 생성 중...");
