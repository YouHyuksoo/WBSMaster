/**
 * @file scripts/import-vietnam-disp-equipment.ts
 * @description
 * 베트남 LGD(DISP) 설비정보 엑셀 파일을 읽어서 Equipment 테이블에 임포트합니다.
 *
 * 엑셀 구조:
 * - 1행: "공정" 컬럼 = 라인 시작점, 다음 컬럼에 라인코드 (1 LINE, 2 LINE 등)
 * - 2행: 컬럼 타이틀 (SMT, 순서, 장비모델, 명칭, H/W 위치, 크림솔더(순서), Maker)
 * - 3행부터: 실제 설비 데이터
 *
 * 실행 방법:
 * npx tsx -r dotenv/config scripts/import-vietnam-disp-equipment.ts <projectId> <excelFilePath> dotenv_config_path=.env.local
 */
...
function inferEquipmentType(name: string): EquipmentType {
  ...
  // SMD 제조 설비
  if (nameLower.includes("printer") || nameLower.includes("s/printer")) {
  ...
  if (nameLower.includes("moi")) {
    return "MOI";
  }
  if (nameLower.includes("loader")) {
    return "LOADER";
  }
  if (nameLower.includes("conveyor") || nameLower.includes("cooling") || nameLower.includes("slide")) {
    return "CONVEYOR";
  }
  if (nameLower.includes("buffer") || nameLower.includes("slider") || nameLower.includes("gate")) {
    return "STORAGE";
  }
  if (nameLower.includes("router") || nameLower.includes("cutting")) {
    return "ROUTER";
  }
  if (nameLower.includes("marking") || nameLower.includes("label")) {
    return "PID_MARKING";
  }
  if (nameLower.includes("turn")) {
    return "CONVEYOR";
  }
  if (nameLower.includes("work table")) {
    return "DEVICE";
  }
  if (nameLower.includes("ict")) {
    return "ICT";
  }
  if (nameLower.includes("fct")) {
    return "FCT";
  }
...
function generateEquipmentCode(lineCode: string, index: number): string {
  // 라인코드에서 공백 제거하고 정규화
  const sanitizedLine = lineCode.replace(/\s+/g, "_").replace(/_LINE/i, "").toUpperCase();
  return `EQ-DISP-L${sanitizedLine}-${String(index).padStart(3, "0")}`;
}

/** 라인코드 정규화 함수 */
function normalizeLineCode(lineCode: string): string {
  // "1 LINE" -> "L1", "2 LINE" -> "L2"
  const match = lineCode.match(/(\d+)\s*LINE/i);
  if (match) {
    return `L${match[1]}`;
  }
...
async function importEquipment(projectId: string, filePath: string) {
  console.log("=== 베트남 DISP(LGD) 설비정보 임포트 시작 ===");
  ...
  for (let i = 0; i < lineStartColumns.length; i++) {
    ...
    // 라인 코드 읽기 (1행, 다음 컬럼) - "1 LINE", "2 LINE" 등
    ...
    console.log(`\n=== 라인: ${lineCode} (${rawLineCode}, 공정: ${processType}) ===`);

    // 컬럼 인덱스 (순서, 장비모델, 명칭, H/W 위치, 크림솔더, Maker)
    const colIndex = {
      order: lineStartCol + 1, // 순서
      model: lineStartCol + 2, // 장비모델
      name: lineStartCol + 3, // 명칭
      hwLocation: lineStartCol + 4, // H/W 위치
      solder: lineStartCol + 5, // 크림솔더(순서)
      maker: lineStartCol + 6, // Maker
    };
    ...
    for (let row = 2; row <= range.e.r; row++) {
      ...
      // H/W 위치
      const hwLocationCell = worksheet[XLSX.utils.encode_cell({ r: row, c: colIndex.hwLocation })];
      const hwLocation = hwLocationCell ? String(hwLocationCell.v).trim() : "";
      ...
      try {
        ...
        await prisma.equipment.create({
          data: {
            ...
            location: hwLocation || processType || null,
            lineCode: lineCode,
            divisionCode: "V_DISP", // 사업부: V_DISP
            projectId,
...
async function main() {
  ...
  if (args.length < 2) {
    console.error(
      "사용법: npx tsx -r dotenv/config scripts/import-vietnam-disp-equipment.ts <projectId> <excelFilePath> dotenv_config_path=.env.local"
    );
    process.exit(1);
  }
