/**
 * @file scripts/import-as-is-vivi.ts
 * @description
 * V_IVI 사업부 AS-IS 분석 데이터 가져오기 스크립트
...
const BUSINESS_UNIT = "V_IVI";

// 대분류 매핑 (엑셀 -> Enum)
const MAJOR_CATEGORY_MAP: Record<string, string> = {
  "기준관리": "MASTER",   // 기준관리 -> MASTER로 변경!
  "생산관리": "PRODUCTION",
  "품질관리": "QUALITY",
  "자재관리": "MATERIAL",
  "설비관리": "EQUIPMENT",
...
async function main() {
  console.log("🚀 V_IVI AS-IS 데이터 가져오기 시작...\n");

  // 1. 엑셀 파일 읽기
  const workbook = XLSX.readFile("D:/Download/2_1_2_1.프로세스 맵핑_251217_jylee_V2 (1).xlsx");
  const sheet = workbook.Sheets["V_IVI"];
  ...
  for (let i = 2; i < rawData.length; i++) {
    ...
    items.push({
      ...
      majorCategory: MAJOR_CATEGORY_MAP[currentMajor] || "OTHER",
  ...
  if (!overview) {
    console.log("📁 V_IVI Overview 생성 중...");
