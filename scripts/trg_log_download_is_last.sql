CREATE OR REPLACE TRIGGER TRG_LOG_DOWNLOAD_IS_LAST
BEFORE INSERT ON LOG_DOWNLOAD
FOR EACH ROW
DECLARE
  lvd_inspect_date     date;
BEGIN

-------------------------------------------------------------------------------
-- 작업일자 / 근무조 / 타임존 자동 설정
-- DOWNLOAD 는 DAY(YYYY-MM-DD) 와 LOG_TIME(HH24:MI:SS) 이 분리되어 저장됨
-- LOG_TIME 이 NULL 이면 날짜만 사용 (시간은 00:00:00)
-------------------------------------------------------------------------------
  if :NEW.DAY is not null then
     if :NEW.LOG_TIME is not null then
        lvd_inspect_date := to_date(:NEW.DAY || ' ' || :NEW.LOG_TIME, 'YYYY-MM-DD HH24:MI:SS');
     else
        lvd_inspect_date := to_date(:NEW.DAY, 'YYYY-MM-DD');
     end if;
     :NEW.ACTUAL_DATE := f_get_work_actual_date(lvd_inspect_date, 'A');
     :NEW.SHIFT_CODE  := f_get_work_shift_code(lvd_inspect_date);
     :NEW.ZONE_CODE   := f_get_worktime_zone_hour(lvd_inspect_date);
  end if;

  UPDATE LOG_DOWNLOAD
  SET IS_LAST = 'N'
  WHERE BARCODE = :NEW.BARCODE
    AND IS_LAST = 'Y';
  :NEW.IS_LAST := 'Y';

  IF :NEW.RESULT IS NOT NULL
     AND UPPER(:NEW.RESULT) NOT IN ('PASS','OK','GOOD','Y') THEN
    P_AUTO_INSERT_QC(:NEW.BARCODE, :NEW.LOG_ID, :NEW.EQUIPMENT_ID, 'W130', :new.file_name);
  END IF;
END;
/
